...
Coding agents 需要的不只是 chat window。它们需要 file browser、code viewer 和 diff panel，也就是一种 IDE experience。这个 pattern 会把 deep agent 连接到 [sandbox](/oss/javascript/deepagents/sandboxes)，让它可以在 isolated environment 中 read、write 和 execute code，然后通过 custom API server 暴露 sandbox filesystem，让 frontend 可以在 agent 工作时实时显示 files。
