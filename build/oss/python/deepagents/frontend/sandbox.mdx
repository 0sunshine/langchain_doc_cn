...
Coding agents 需要的不只是 chat window。它们需要 file browser、code viewer 和 diff panel，也就是一种 IDE experience。这个 pattern 会把 deep agent 连接到 [sandbox](/oss/python/deepagents/sandboxes)，让它可以在 isolated environment 中 read、write 和 execute code，然后通过 custom API server 暴露 sandbox filesystem，让 frontend 可以在 agent 工作时实时显示 files。
...
Deep Agents 支持多个 [sandbox providers](/oss/python/integrations/sandboxes)。任何实现 `SandboxBackendProtocol` 的 provider 都可以使用：
...
IDE layout 会把三个 panels 并排排列：

| Panel       | Width         | Purpose                                      |
| ----------- | ------------- | -------------------------------------------- |
| File tree   | Fixed (208px) | 浏览 sandbox files，查看 change indicators   |
| Code / Diff | Flexible      | 查看 file content 或 unified diff            |
| Chat        | Fixed (320px) | 与 agent 交互                                |
