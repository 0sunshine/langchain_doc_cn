...
这建立在 [subagents](/oss/python/deepagents/subagents) 之上；普通 subagents 是同步运行的，会阻塞 supervisor 直到完成。当任务运行时间较长、可并行化，或需要在执行过程中进行引导时，请使用 async subagents。

<Note>
Async subagents 是 `deepagents` 0.5.0 中提供的 preview feature。Preview features 正在积极开发中，API 可能会变化。
...
将 async subagents 定义为一组 [`AsyncSubAgent`](https://reference.langchain.com/python/deepagents/middleware/async_subagents/AsyncSubAgent) specs，每个 spec 都指向一个 Agent Protocol server：
...
|-------|------|-------------|
| `name` | `str` | 必填。唯一标识符。supervisor 启动任务时会使用它。 |
| `description` | `str` | 必填。说明这个 subagent 做什么。supervisor 用它判断要 delegate 给哪个 agent。 |
| `graph_id` | `str` | 必填。Agent Protocol server 上的 graph ID（或 assistant ID）。对于基于 LangGraph 的 deployments，它必须匹配 `langgraph.json` 中注册的 graph。 |
| `url` | `str` | 可选。省略时使用 ASGI transport（in-process）。设置后使用 HTTP transport 连接远程 Agent Protocol server。 |
| `headers` | `dict[str, str]` | 可选。发往远程 server 的额外 headers。可用于 self-hosted Agent Protocol servers 的自定义 authentication。 |
...
## 使用 async subagent tools

[`AsyncSubAgentMiddleware`](https://reference.langchain.com/python/deepagents/middleware/async_subagents/AsyncSubAgentMiddleware) 会向 supervisor 提供五个 tools：

| Tool | Purpose | Returns |
|------|---------|---------|
| `start_async_task` | 启动新的后台任务 | Task ID（立即返回） |
| `check_async_task` | 获取任务当前状态和结果 | Status + result（如果已完成） |
| `update_async_task` | 向正在运行的任务发送新指令 | Confirmation + updated status |
| `cancel_async_task` | 停止正在运行的任务 | Confirmation |
| `list_async_tasks` | 列出所有已跟踪任务及实时状态 | 所有任务的 summary |
...
Task metadata 存储在 supervisor graph 的专用 state channel（`async_tasks`）中，与 message history 分离。这一点很关键，因为当 context window 填满时，deep agents 会[压缩它们的 message history](/oss/python/deepagents/context-engineering#summarization)。如果 task ID 只存在于 tool messages 中，它们会在 compaction 期间丢失。专用 channel 确保 supervisor 即使经过多轮 summarization 后，也始终能通过 `list_async_tasks` 回忆起自己的任务。

每个被跟踪的 task 都会记录 task ID、agent name、thread ID、run ID、status 和 timestamps（`created_at`、`last_checked_at`、`last_updated_at`）。
...
**Solution**：worker pool 可能已经耗尽。用 `--n-jobs-per-worker` 增大 pool size。参见[设置 worker pool 大小](#为本地开发设置合适的-worker-pool-大小)。
