...
这建立在 [subagents](/oss/deepagents/subagents) 之上；普通 subagents 是同步运行的，会阻塞 supervisor 直到完成。当任务运行时间较长、可并行化，或需要在执行过程中进行引导时，请使用 async subagents。
...
将 async subagents 定义为一组 @[`AsyncSubAgent`] specs，每个 spec 都指向一个 Agent Protocol server：
...
@[`AsyncSubAgentMiddleware`] 会向 supervisor 提供五个 tools：
