...
[sandboxes](/oss/deepagents/sandboxes) 是一种 code-first 的方式，用于作用于某个 environment（例如运行命令、安装 dependencies、编辑文件）；而 interpreters 是一种 code-first 的方式，用于作用在 agent loop 内部：组合 tools、保留 state，并决定哪些信息应返回给 model。
...
| 跨 threads 复用的 helpers | 带 [interpreter skills](/oss/deepagents/skills#use-interpreter-skills) 的 Interpreter |
| Shell commands、package installs、tests，或完整 OS filesystem access | [Sandboxes](/oss/deepagents/sandboxes) |
...
Interpreter skills 是向 interpreter 暴露 code modules 的 [skills](/oss/deepagents/skills)。配置 interpreter middleware 后，agent 可以从代码中 import 这些 modules，并将其用于确定性的 helper logic。

当 agent 需要为 structured data workflows 复用 helpers（例如 sorting、grouping、scoring、parsing、validating 或 aggregating data）时，interpreter skills 很有用。设置细节请参见 [Interpreter skills](/oss/deepagents/skills#use-interpreter-skills)。
...
当 graph 使用 checkpointer 时，它会与 [LangGraph time travel](/oss/langgraph/use-time-travel) 配合。恢复 graph checkpoint 可以恢复存储在 graph state 中的 interpreter snapshot，因此你可以在 debugging 或 replay 时回到更早的 agent context 和 interpreter state。
...
| Filesystem access | No | 通过 PTC allowlist 添加 [built-in filesystem tools](/oss/deepagents/harness#virtual-filesystem-access) |
| Network access | No | 通过 PTC 暴露一个具体 network tool |
| Wall-clock or datetime access | No | 如有需要，暴露一个显式 time tool |
| Shell commands、package installs、tests、OS-level execution | No | 使用 [sandbox backend](/oss/deepagents/sandboxes) |
:::
:::js
| Capability | Available by default | How to expose it |
| --- | --- | --- |
