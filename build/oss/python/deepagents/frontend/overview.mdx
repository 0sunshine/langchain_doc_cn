...
[LangChain frontend patterns](/oss/python/langchain/frontend/overview) 中的 markdown messages、tool calling 和 human-in-the-loop 等模式，也都适用于 deep agents。Deep Agents 构建在同一个 LangGraph runtime 之上，因此 `useStream` 提供相同的核心 API。
