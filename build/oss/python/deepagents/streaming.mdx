...
<Tip>
对于新应用，我们推荐使用 [event streaming](/oss/python/deepagents/event-streaming)，也就是 Deep Agents v0.6 引入的 typed-projection API。Event streaming 会为每个 projection（subagents、messages、tool calls、values）提供独立 iterators，让你可以分别消费它们，而不必基于 `stream_mode` chunks 分支处理。
</Tip>
...
在 subagent tools 内部使用 [`get_stream_writer`](https://reference.langchain.com/python/langgraph/config/get_stream_writer) 发出 custom progress events：
...
## v2 streaming 格式 {#v2-streaming-format}
...
关于 v2 format 的更多细节，包括 type narrowing 和 Pydantic/dataclass coercion，请参见 [LangGraph streaming docs](/oss/python/langgraph/streaming#stream-output-format-v2)。


## 相关 {#related}

- [Subagents](/oss/python/deepagents/subagents)：配置并使用 Deep Agents 的 subagents
- [Frontend streaming](/oss/python/deepagents/streaming/frontend)：使用 `useStream` 为 Deep Agents 构建 React UIs
- [LangChain Event Streaming](/oss/python/langchain/event-streaming)：LangChain agents 的通用 streaming concepts
