...
本页介绍**同步** subagents：supervisor 会阻塞，直到 subagent 完成。对于 long-running tasks、并行 workstreams，或需要 mid-flight steering 和 cancellation 的场景，请参阅 [Async subagents](/oss/deepagents/async-subagents)。
...
`subagents` 应该是 dictionaries 或 @[`CompiledSubAgent`] objects 的列表。有两种类型：
...
- 如需重命名或重新设置自动添加版本的 prompt，请在当前 [harness profile](/oss/deepagents/profiles#harness-profiles) 上设置 `general_purpose_subagent=GeneralPurposeSubagentProfile(...)`。
...
1. 在当前 [harness profile](/oss/deepagents/profiles#harness-profiles) 上设置 `general_purpose_subagent=GeneralPurposeSubagentProfile(enabled=False)`。
...
Async subagents 不受影响；它们会经过自己的 middleware 和 tools，详见 [Async subagents](/oss/deepagents/async-subagents)。
...
把 subagents 定义为符合 @[`SubAgent`] spec 的 dictionaries，包含以下字段：
...
| `interrupt_on` | `dict[str, bool]` | 可选。为特定 tools 配置 [human-in-the-loop](/oss/deepagents/human-in-the-loop)。Subagent value 会覆盖 main agent。需要 checkpointer。<br></br>默认从 main agent 继承。subagent value 会覆盖默认值。 |
| `skills` | `list[str]` | 可选。[Skills](/oss/deepagents/skills) source paths。指定后，subagent 会从这些 directories 加载 skills（例如 `["/skills/research/", "/skills/web-search/"]`）。这允许 subagents 使用不同于 main agent 的 skill sets。<br></br>不会从 main agent 继承。只有 general-purpose subagent 会继承 main agent 的 skills。当 subagent 拥有 skills 时，它会运行自己的独立 @[`SkillsMiddleware`] instance。Skill state 完全隔离：subagent 加载的 skills 对 parent 不可见，反之亦然。 |
| `response_format` | `ResponseFormat` | 可选。subagent 的 [Structured output](/oss/langchain/structured-output) schema。设置后，parent 接收的是 subagent 结果的 JSON，而不是 free-form text。接受 Pydantic models、`ToolStrategy(...)`、`ProviderStrategy(...)` 或 raw schema type。参阅 [Structured output](#structured-output)。 |
| `permissions` | `list[FilesystemPermission]` | 可选。subagent 的 [Filesystem permission rules](/oss/deepagents/permissions)。设置后会**完全替换** parent agent 的 permissions。<br></br>默认从 main agent 继承。 |
...
| `interrupt_on` | `dict[str, bool]` | 可选。为特定 tools 配置 [human-in-the-loop](/oss/deepagents/human-in-the-loop)。Subagent value 会覆盖 main agent。需要 checkpointer。<br></br>默认从 main agent 继承。subagent value 会覆盖默认值。 |
| `skills` | `list[str]` | 可选。[Skills](/oss/deepagents/skills) source paths。指定后，subagent 会从这些 directories 加载 skills（例如 `["/skills/research/", "/skills/web-search/"]`）。这允许 subagents 使用不同于 main agent 的 skill sets。<br></br>不会从 main agent 继承。只有 general-purpose subagent 会继承 main agent 的 skills。当 subagent 拥有 skills 时，它会运行自己的独立 @[`SkillsMiddleware`] instance。Skill state 完全隔离：subagent 加载的 skills 对 parent 不可见，反之亦然。 |
| `responseFormat` | `ResponseFormat` | 可选。subagent 的 [Structured output](/oss/langchain/structured-output) schema。设置后，parent 接收的是 subagent 结果的 JSON，而不是 free-form text。接受 Zod schemas、JSON schema objects、`toolStrategy(...)` 或 `providerStrategy(...)`。参阅 [Structured output](#structured-output)。 |

:::

### CompiledSubAgent

对于复杂 workflows，可以使用预先构建的 LangGraph graph 作为 @[`CompiledSubAgent`]：
...
对于更复杂的 use cases，你可以使用 @[`CompiledSubAgent`] 提供 custom subagents。
你可以使用 LangChain 的 @[`create_agent`] 创建 custom subagent，或用 [graph API](/oss/langgraph/graph-api) 创建 custom LangGraph graph。

如果你创建 custom LangGraph graph，请确保 graph 有一个名为 `"messages"` 的 [state key](/oss/langgraph/quickstart#2-define-state)：
...
Subagents 支持 [structured output](/oss/langchain/structured-output)，因此 parent agent 会收到可预测、可解析的 JSON，而不是 free-form text。
...
在 subagent config 上传入 `response_format`。当 subagent 完成时，它的 structured response 会被 JSON-serialized，并作为 `ToolMessage` content 返回给 parent agent。schema 接受 @[`create_agent`] 支持的任何内容：Pydantic models、`ToolStrategy(...)`、`ProviderStrategy(...)` 或 raw schema type。
...
关于 schema types 和 strategies（tool calling vs. provider-native）的完整细节，请参阅 [Structured output](/oss/langchain/structured-output)。
...
使用 `create_deep_agent` 配置 [skills](/oss/deepagents/skills) 时：
...
当你使用 [runtime context](/oss/langchain/runtime) invoke parent agent 时，该 context 会自动传播到所有 subagents。每个 subagent run 都会接收你在 parent `invoke` / `ainvoke` call 上传入的相同 runtime context。
