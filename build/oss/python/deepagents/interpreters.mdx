...
[sandboxes](/oss/python/deepagents/sandboxes) 是一种 code-first 的方式，用于作用于某个 environment（例如运行命令、安装 dependencies、编辑文件）；而 interpreters 是一种 code-first 的方式，用于作用在 agent loop 内部：组合 tools、保留 state，并决定哪些信息应返回给 model。
...
| 跨 threads 复用的 helpers | 带 [interpreter skills](/oss/python/deepagents/skills#use-interpreter-skills) 的 Interpreter |
| Shell commands、package installs、tests，或完整 OS filesystem access | [Sandboxes](/oss/python/deepagents/sandboxes) |

## 向 agent 添加 interpreter
...
默认情况下，interpreter state 还会在同一个 thread 的不同 turns 之间持久化：每次 agent run 之后 snapshot working state，并在下一次 run 前恢复。
...
Interpreter skills 是向 interpreter 暴露 code modules 的 [skills](/oss/python/deepagents/skills)。配置 interpreter middleware 后，agent 可以从代码中 import 这些 modules，并将其用于确定性的 helper logic。

当 agent 需要为 structured data workflows 复用 helpers（例如 sorting、grouping、scoring、parsing、validating 或 aggregating data）时，interpreter skills 很有用。设置细节请参见 [Interpreter skills](/oss/python/deepagents/skills#use-interpreter-skills)。

## Snapshots 和 time travel

默认情况下，`CodeInterpreterMiddleware` 会在每次 agent run 之后 snapshot interpreter state，并在下一次 run 前恢复它。snapshot 是 interpreter 的 in-memory JavaScript state 的序列化副本，包括 agent 完成代码运行时存在的 globals、variables、functions 和 imported modules。

跨 conversation turns 的 lifecycle 是：

1. 一个 turn 开始，`CodeInterpreterMiddleware` 会为该 thread 恢复最新的 interpreter snapshot。
2. agent 调用 `eval`，代码可以读取或修改 interpreter variables。
3. agent run 完成，middleware 会把更新后的 interpreter state snapshot 到 graph state 中。
4. 下一个 turn 会从恢复后的 interpreter state 开始，而不是从空 runtime 开始。

在单个 agent run 内，重复的 `eval` calls 会使用 live interpreter context object。middleware 不会在这些调用之间 snapshot 和 restore；它会在 run 完成时 snapshot context，以便在后续 turn 或 checkpoint replay 中恢复。

<Note>
    在 conversation turns 之间，snapshots 只会保留可以合理序列化的值。请把它们用于数据，而不是 live runtime objects。Functions、classes 和其他不可序列化的值会被恢复为 inaccessible artifacts。如果 interpreter code 在 restore 后访问其中一个，eval tool 会抛出类似这样的错误：`Value for 'fn' was not restored because it is not serializable (type: function).`
</Note>

Snapshots 会保留 interpreter memory，而不是外部世界 effects。如果 interpreter code 通过 PTC 调用 tool，恢复之前的 interpreter snapshot 不会撤销该 tool call 的 side effects。它只会恢复记录或处理该结果的 interpreter variables。

当 graph 使用 checkpointer 时，它会与 [LangGraph time travel](/oss/python/langgraph/use-time-travel) 配合。恢复 graph checkpoint 可以恢复存储在 graph state 中的 interpreter snapshot，因此你可以在 debugging 或 replay 时回到更早的 agent context 和 interpreter state。
...
你可以用 `snapshot_between_turns=False` 禁用跨 turn snapshots。
...
| `console.log` capture | Yes | 用 `capture_console=False` 禁用 |
| Agent tools | No | 添加 PTC allowlist |
| Interpreter skill modules | No | 添加 `module` entry，并配置 `skills_backend` 或 `skillsBackend` |
| Filesystem access | No | 通过 PTC allowlist 添加 [built-in filesystem tools](/oss/python/deepagents/harness#virtual-filesystem-access) |
| Network access | No | 通过 PTC 暴露一个具体 network tool |
| Wall-clock or datetime access | No | 如有需要，暴露一个显式 time tool |
| Shell commands、package installs、tests、OS-level execution | No | 使用 [sandbox backend](/oss/python/deepagents/sandboxes) |
...
<Note>
    **代码执行的工作方式**
    ...
    Interpreter code 运行在 embedded QuickJS context 中，不是在单独的 VM 或 process 中。在 Python 中，这个 runtime 由 [`quickjs-rs`](https://github.com/langchain-ai/quickjs-rs) 提供；它在 [Security guide](https://github.com/langchain-ai/quickjs-rs#security) 中说明了 same-process execution boundary。

    请把 interpreters 视为 capability-scoped execution layer，而不是 host-memory isolation boundary。对于不可信或半可信代码，请在 isolated worker processes 或 containers 中运行 agents，并保持 PTC allowlist 狭窄。
</Note>
...
| --- | --- | --- |
| `memory_limit` | `64 * 1024 * 1024` <br/>(64 MB) | QuickJS heap memory limit，单位为 bytes。 |
| `timeout` | `5.0` | 每次 eval 的 timeout，单位为秒。 |
| `max_ptc_calls` | `256` | 每次 eval 允许的最大 `tools.*` calls 数量。只有在可信环境中才使用 `None`。 |
| `tool_name` | `"eval"` | 暴露给 model 的 interpreter tool 名称。 |
| `max_result_chars` | `4000` | result 和 stdout blocks 返回的最大字符数。 |
| `capture_console` | `True` | 是否捕获 `console.log`、`console.warn` 和 `console.error` 输出。 |
| `ptc` | `None` | PTC allowlist：tool names 或 `BaseTool` instances 的列表。 |
| `skills_backend` | `None` | 用于解析 interpreter skill modules 的 backend。 |
| `snapshot_between_turns` | `True` | interpreter state snapshots 是否跨 agent turns 持久化。 |
| `max_snapshot_bytes` | `None` | 最大 serialized snapshot 大小。默认与 `memory_limit` 相同。 |
